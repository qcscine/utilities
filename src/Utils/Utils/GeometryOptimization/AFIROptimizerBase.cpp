...
void AFIROptimizerBase::evaluateArtificialForces(const AtomCollection& atoms, double& energy, Eigen::MatrixXd& gradients) const {
  // clang-format off
  ...
  const double prefactorWeak = gamma / ((pow(2.0, -1.0 / 6.0) - pow(tmpWeak, -1.0 / 6.0)) * r0);
  ...
  if (this->lhsList.size() > 0 && this->rhsList.size() > 0) {
    ...
    for (unsigned int i = 0; i < this->lhsList.size(); i++) {
      for (unsigned int j = 0; j < this->rhsList.size(); j++) {
        ...
        double radi = covalentRadii[static_cast<unsigned int>(elements[lhs[i]])] * 0.0188971616463207;
        double radj = covalentRadii[static_cast<unsigned int>(elements[rhs[j]])] * 0.0188971616463207;
  ...
  if (this->weak) {
    ...
    for (unsigned int i = 0; i < nAtoms; i++) {
      for (unsigned int j = 0; j < i; j++) {
        ...
        double radi = covalentRadii[static_cast<unsigned int>(elements[i])] * 0.0188971616463207;
        double radj = covalentRadii[static_cast<unsigned int>(elements[j])] * 0.0188971616463207;
