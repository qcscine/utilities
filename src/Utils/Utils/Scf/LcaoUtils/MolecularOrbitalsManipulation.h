...
#ifndef UTILS_MOLECULARORBITALSORBITALSMANIPULATION_H
#define UTILS_MOLECULARORBITALSORBITALSMANIPULATION_H
 ...
 public:
  ...
  struct DeprecatedSwap {
    ...
    DeprecatedSwap(int lowLyingOrbitalNotToFill, int highLyingOrbitalToFill)
      : lowLyingOrbitalNotToFill_(lowLyingOrbitalNotToFill), highLyingOrbitalToFill_(highLyingOrbitalToFill) {
    }
  };
  ...
  struct DeprecatedMix {
    ...
    DeprecatedMix(int lowLyingOrbitalNotToFill, int highLyingOrbitalToFill, double angleInrad)
      : lowLyingOrbitalNotToFill_(lowLyingOrbitalNotToFill),
        highLyingOrbitalToFill_(highLyingOrbitalToFill),
        angleInRad_(angleInrad) {
    }
  };
  /*! Struct to specify swaps.
  ...
  struct Mix {
    ...
    Mix(int orbital1, int orbital2, double angleInrad)
      : orbital1_(orbital1), orbital2_(orbital2), angleInRad_(angleInrad) {
    }
  };

  static void swapRestricted(MolecularOrbitals& orbitals, const std::vector<Swap>& swaps);
  static void swapUnrestricted(MolecularOrbitals& orbitals, const std::vector<Swap>& alphaSwaps,
                               const std::vector<Swap>& betaSwaps);
  ...
  static MolecularOrbitals createUnrestrictedWithSwaps(const MolecularOrbitals& orbitals, const std::vector<Swap>& alphaSwaps,
                                                       const std::vector<Swap>& betaSwaps);
  static MolecularOrbitals createRestrictedWithMixes(const MolecularOrbitals& orbitals, const std::vector<Mix>& mixes);
  static MolecularOrbitals createUnrestrictedWithMixes(const MolecularOrbitals& orbitals, const std::vector<Mix>& alphaMixes,
                                                       const std::vector<Mix>& betaMixes);
...
#endif // UTILS_MOLECULARORBITALSORBITALSMANIPULATION_H