...
void MolecularTrajectoryIO::write(format f, const std::string& fileName, const MolecularTrajectory& m,
  ...
  return write(f, fout, m, bondOrders);
}
...
void MolecularTrajectoryIO::write(format f, std::ostream& out, const MolecularTrajectory& m,
  ...
  else if (f == format::pdb) {
    PdbStreamHandler::writeTrajectory(out, m, bondOrders);
  }
}
...
void MolecularTrajectoryIO::writeXYZLine(std::ostream& out, ElementType e, const Position& p) {
  ...
  out << right << setw(16) << fixed << setprecision(10) << p.x() * Constants::angstrom_per_bohr << " " << right
      << setw(16) << fixed << setprecision(10) << p.y() * Constants::angstrom_per_bohr << " " << right << setw(16)
      << fixed << setprecision(10) << p.z() * Constants::angstrom_per_bohr << endl;
}
...
MolecularTrajectory MolecularTrajectoryIO::readPdb(std::istream& in) {
  PdbStreamHandler pdbStreamHandler;
  pdbStreamHandler.setReadH(true);
  std::vector<AtomCollection> structures = pdbStreamHandler.read(in);
  const ElementTypeCollection elements = structures[0].getElements();
  MolecularTrajectory trajectory(elements);
  trajectory.setResidues(structures[0].getResidues());
  for (auto& atomCollection : structures) {
    trajectory.push_back(atomCollection.getPositions());
    // Clear the atom collection to avoid using twice the memory.
    atomCollection.clear();
  }
  return trajectory;
}
void MolecularTrajectoryIO::write(MolecularTrajectoryIO::format f, const string& fileName, const MolecularTrajectory& m) {
  write(f, fileName, m, BondOrderCollection());
}
