...
class AThermochemistryTest : public Test {
 ...
 protected:
  void SetUp() final {
    ...
    Eigen::Vector3d eigenValues(9.63614964, 1.28657869, 1.13503370);
    // Convert from cm-1 to amu*Angstrom^2
    eigenValues /= 16.8576304198232;
...
TEST_F(AThermochemistryTest, CanCalculateVibrationalComponent) {
  ...
  EXPECT_THAT(container.vibrationalComponent.temperature, DoubleNear(298.00, 1e-5));
...
TEST_F(AThermochemistryTest, CanCalculateRotationalComponentHF) {
  ...
  Eigen::Vector3d eigenValues(0.00000000, 18.89052201, 18.89052201);
  // Convert from cm-1 to amu*Angstrom^2
  eigenValues /= 16.8576304198232;
  ...
  ASSERT_THAT(container.rotationalComponent.entropy * Constants::kCalPerMol_per_hartree, DoubleNear(6.7459 / 1000, 1e-5));
  ASSERT_THAT(container.rotationalComponent.temperature, DoubleNear(298.00, 1e-5));
}
...
TEST_F(AThermochemistryTest, CanCalculateRotationalComponentOfAsymmetricMolecule) {
  ...
  arbitraryTCCalculator = std::make_unique<ThermochemistryCalculator>(arbitraryNormalModes, pmi, arbitraryElements,
                                                                      arbitraryMultiplicity, arbitraryEnergy);
  arbitraryTCCalculator->setZPVEInclusion(ZPVEInclusion::alreadyIncluded);
  arbitraryTCCalculator->setTemperature(298.00);
  // C2v symmetry
  ...
  ASSERT_THAT(container.rotationalComponent.entropy * Constants::kCalPerMol_per_hartree, DoubleNear(16.0111 / 1000, 1e-5));
  ASSERT_THAT(container.rotationalComponent.temperature, DoubleNear(298.00, 1e-5));
}
...
TEST_F(AThermochemistryTest, CanCalculateTranslationalComponentOfAsymmetricMolecule) {
  ...
  ASSERT_THAT(container.translationalComponent.temperature, DoubleNear(298.00, 1e-5));
}
...
TEST_F(AThermochemistryTest, CanCalculateOverallComponent) {
  ...
  ASSERT_THAT(container.overall.temperature, DoubleNear(298.00, 1e-5));
}
} // namespace Utils
} // namespace Scine